 ...
 */
/obj/proc/wrenchAnchor(var/mob/user, var/obj/item/weapon/wrench/W, var/time_to_wrench = 3 SECONDS) //proc to wrench an object that can be secured
	...
	if(do_after(user, src, time_to_wrench * W.toolspeed))
		if(!canAffixHere(user))
			return FALSE
		anchored = !anchored

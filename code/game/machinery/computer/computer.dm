...
/obj/machinery/computer/togglePanelOpen(var/obj/item/weapon/screwdriver/toggleitem, mob/user, var/obj/item/weapon/circuitboard/CC = null)
	if(!circuit) //we can't disassemble with no circuit, so add some fucking circuits if you want disassembly
		return
	playsound(src, toggleitem.usesound, 50, 1)

...
/obj/machinery/alarm/attackby(obj/item/W as obj, mob/user as mob)
	...
	switch(buildstage)
		if(2)
			...
			if(wiresexposed && !wires.IsAllCut() && iswiretool(W, user))
...
//			else if(wiresexposed && wires.IsAllCut() && iswirecutter(W))
		...
		if(1)
			if(iscablecoil(W))
				...
				playsound(src, coil.usesound, 50, 1)
				coil.use(5)
				buildstage = 2
				update_icon()
				first_run()
				...
				playsound(src, W.usesound, 50, 1)
				if(do_after(user, src, 20 * W.toolspeed) && buildstage == 1)
...
//				if(iswirecutter(W))
				if(W.is_wirecutter(user))
					...
					playsound(src, W.usesound, 50, 1)
					if (do_after(user, src,  50 * W.toolspeed) && buildstage == 2 && wiresexposed)
						buildstage=1
						user.visible_message("<span class='attack'>[user] has cut the wiring from \the [src]!</span>", "You have cut the last of the wiring from \the [src].")
						update_icon()
						getFromPool(/obj/item/stack/cable_coil, get_turf(user), 5)
			if(1)
				if(iscablecoil(W))
					...
					to_chat(user, "You start prying out the circuit...")
					playsound(src, W.usesound, 50 * W.toolspeed, 1)
